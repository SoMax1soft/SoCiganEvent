...
public class VillagerListener implements Listener {
    ...
    public VillagerListener(Main plugin, WorldGuardPlugin worldGuard, Essentials essentials, VillagerSpawnTask villagerSpawnTask) {
        this.plugin = plugin;
    ...
    public void onPlayerInteractVillager(PlayerInteractEntityEvent event) {
        if (event.getRightClicked() instanceof Villager) {
            ...
            if (villager.getCustomName() != null && villager.getCustomName().contains(config.getString("Villager.Name"))) {
                ...
                synchronized (lock) {
                    ...
                    meta.setDisplayName(config.getString("Villager.coinname"));
                    ...
                    sendMessageToNearbyPlayers(villager.getLocation(), ChatColor.translateAlternateColorCodes('&', config.getString("Messages.CoinTaken").replace("%player%", player.getName())));
                            ...
                            synchronized (lock) {
                                if (villagerSpawnTask.enabled) {
                                    if (player.isOnline() && player.getInventory().contains(emerald)) {
                                        if (player.getLocation().distance(villager.getLocation()) > config.getInt("Villager.Distance")) {
                                            sendMessageToNearbyPlayers(villager.getLocation(), ChatColor.translateAlternateColorCodes('&', config.getString("Messages.TooFar").replace("%player%", player.getName())));
                                        ...
                                        sendMessageToNearbyPlayers(villager.getLocation(), ChatColor.translateAlternateColorCodes('&', config.getString("Messages.State").replace("%player%", player.getName()).replace("%state%", "выкинул монетку")));
                                        player.getInventory().remove(emerald);
                                        villager.getEquipment().setItemInMainHand(emerald);
                                        playerCoinStatus.put(playerUUID, false);
                                        isCoinTaken = false;
    ...
    public void onPlayerDropItem(PlayerDropItemEvent event) {
        ...
        if (item != null && item.hasItemMeta() && item.getItemMeta().hasDisplayName() &&
                item.getItemMeta().getDisplayName().equals(config.getString("Villager.coinname"))) {
            ...
            synchronized (lock) {
                if (playerCoinStatus.getOrDefault(playerUUID, false)) {
                    ...
                    if (!dropMessageSent.getOrDefault(playerUUID, false)) {
                        sendMessageToNearbyPlayers(player.getLocation(), ChatColor.translateAlternateColorCodes('&', config.getString("Messages.State")
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        for (ItemStack item : event.getDrops()) {
            if (item != null && item.hasItemMeta()) {
                ...
                if (meta != null && meta.hasDisplayName() && meta.getDisplayName().equalsIgnoreCase(config.getString("Villager.coinname"))) {
                    emerald = item;
                    break;
                }
            }
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        if (emerald != null) {
            player.getInventory().remove(emerald);
            sendMessageToNearbyPlayers(player.getLocation(), ChatColor.translateAlternateColorCodes('&',
                    config.getString("Messages.State").replace("%player%", player.getName()).replace("%state%", "ливнул с позором")));
            playerCoinStatus.put(playerUUID, false);
            isCoinTaken = false;
            if (villager != null) {
                if (villager.getEquipment() != null) {
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (isPlayerInYourRegion(player.getLocation())) {
            if (!player.hasPermission(config.getString("Bypass.Permission"))) {
                ...
                player.sendMessage("Заберите монетку у Цыгана.");
    ...
    public void onPlayerChangedWorld(PlayerChangedWorldEvent event) {
        ...
        if (emerald != null) {
            player.getInventory().remove(emerald);
            sendMessageToNearbyPlayers(player.getLocation(), ChatColor.translateAlternateColorCodes('&',
                    config.getString("Messages.State").replace("%player%", player.getName()).replace("%state%", "сменил мир и потерял монетку")));
            playerCoinStatus.put(playerUUID, false);
            isCoinTaken = false;
        }
    }
    ...
    private boolean isPlayerInYourRegion(Location location) {
        String chestRegionName = config.getString("Villager.regioname");
